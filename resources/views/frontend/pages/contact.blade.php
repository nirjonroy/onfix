...
@php
    ...
    $contactPhone = $contact->phone ?: (siteInfo()->topbar_phone ?? '');
    $contactEmail = $contact->email ?: (siteInfo()->contact_email ?? '');
    $contactAddress = $contact->address ?: trim((siteInfo()->address_1 ?? '') . ' ' . (siteInfo()->address_2 ?? ''));
    $contactTitle = $contact->title ?: 'Contact Information';
    $contactDescription = $contact->description ?: 'Get in touch with our repair team for diagnostics, pricing, and turnaround details.';
    $socialLinks = [
                        ...
                        <div class="container-fluid">
                            <div class="row">
                                <div class="col-md-12 p-0">
                                    <div class="map">
                                        <iframe src="{{ $contact->map }}" height="600" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="row wrap-our-footer style-1">
                                    <div class="col-md-4">
                                        <div class="our-inner bg-style-2">
                                            <a class="icons color-w pt-style-1 background-color-042c71" href="{{ $contactPhone ? 'tel:' . preg_replace('/[^0-9+]/', '', $contactPhone) : '#' }}">
                                                <span class="icon-mobile-keyboard text-white"></span>
                                            </a>
                                            <ul class="list-info">
                                                <li class="title teko"><a class="text-white" href="{{ $contactPhone ? 'tel:' . preg_replace('/[^0-9+]/', '', $contactPhone) : '#' }}">Our Phone</a></li>
                                                <li class="sub-title pad-1"><a class="text-white" href="{{ $contactPhone ? 'tel:' . preg_replace('/[^0-9+]/', '', $contactPhone) : '#' }}">{{ $contactPhone }}</a></li>
                                            </ul>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="our-inner pl-style-2 bg-style-1">
                                            <a class="icons pt-style-2 background-color-ff4e4e" href="{{ $contactEmail ? 'mailto:' . $contactEmail : '#' }}">
                                                <span class="font-size-33 text-white icon-mail-open"></span>
                                            </a>
                                            <ul class="list-info mar-info-1">
                                                <li class="title teko mt-1"><a class="text-white" href="{{ $contactEmail ? 'mailto:' . $contactEmail : '#' }}">Our Email</a></li>
                                                <li class="sub-title pad-1"><a class="text-white" href="{{ $contactEmail ? 'mailto:' . $contactEmail : '#' }}">{{ $contactEmail }}</a></li>
                                            </ul>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
